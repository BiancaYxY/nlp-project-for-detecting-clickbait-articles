...
class SemanticSimilarityService:
    ...
    # ── Language detection ────────────────────────────────────────────────────
    def _detect_language(self, headline: str, article_text: str) -> str:
        text = f"{headline} {article_text}".strip()
        ...
        try:
            lang = detect(text)
            return "ro" if lang.startswith("ro") else "en"
    ...
    # ── Main compute ─────────────────────────────────────────────────────────
    def compute(self, headline: str, article_text: str, language: str = None) -> Dict:
        ...
        # detectie automata daca nu e data limba
        detected_language = language or self._detect_language(headline, article_text)

        model = self._select_model(detected_language)
        ...
        return {
            ...
            "model_used": self.ro_model_name if detected_language == "ro" else self.en_model_name,
...
def compute_similarity(scraping_data: Dict, language: str = None) -> Dict:
